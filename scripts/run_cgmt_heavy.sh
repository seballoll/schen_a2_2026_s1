...
# Usage:
#   ./scripts/run_cgmt_heavy.sh [runner] [out_dir] [runs] [steps] [particles] [thread_counts_csv]
# Example:
#   ./scripts/run_cgmt_heavy.sh "build_root_test/Proyecto Individual/lab_runner" \
#       benchmark_artifacts/cgmt_heavy 12 50 1200 "2,4,6,8,10,12,14,16,18"

RUNNER="${1:-build_root_test/Proyecto Individual/lab_runner}"
OUT_DIR="${2:-benchmark_artifacts/cgmt_heavy}"
RUNS="${3:-12}"
STEPS="${4:-50}"
PARTICLES="${5:-1200}"
...
mkdir -p "$OUT_DIR"
RAW_CSV="$OUT_DIR/raw_cgmt_runs.csv"
SUMMARY_CSV="$OUT_DIR/summary_cgmt.csv"
PLOT_PNG="$OUT_DIR/cgmt_heavy_trend.png"
...
echo "Running heavy CGMT benchmark..."
echo "- runner: $RUNNER"
echo "- runs: $RUNS"
echo "- steps: $STEPS"
...
for t in "${THREADS[@]}"; do
  ...
  echo "  -> cgmt_t$t_trimmed"
  for ((i=0; i<RUNS; i++)); do
    run_and_capture "chunked" "$t_trimmed" "$i"
  done
done
...
if not threads:
    raise SystemExit("No CGMT (chunked) data found in summary CSV.")
...
plt.plot(threads, cycles, marker="s", linewidth=2.0, label="CGMT (chunked)")
...
plt.title("Heavy CGMT cycle trend")
